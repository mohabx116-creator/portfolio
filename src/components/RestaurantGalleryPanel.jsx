...
function RestaurantGalleryPanel() {
        ...
        <div className="dashboard-card ghost-border overflow-hidden rounded-[1.75rem] p-5 sm:p-6 lg:p-7">
          <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
            <div className="text-start">
              <p className="font-label text-xs uppercase tracking-[0.22em] text-primary">{projectsSection.restaurantTab || 'Restaurant Gallery'}</p>
              ...
              <div className="mt-6">
                ...
                >
                  {restaurantGallery.caseStudyCta}
                </Link>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {screens.map((screen) => (
                <a
                  key={screen.id}
                  href={`${restaurantGallery.route}#${screen.id}`}
                ...
                >
                  <div className="overflow-hidden rounded-[0.95rem]">
                    <img src={screen.image} alt={screen.title} className="aspect-[4/3] w-full object-cover object-top transition-transform duration-500 group-hover:scale-[1.03]" />
                  </div>
      ...
      <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4 xl:gap-6">
        {screens.map((screen, index) => (
          <Reveal key={screen.id} delay={index * 0.08}>
            ...
            >
              <div className="relative h-52 overflow-hidden bg-surface-high sm:h-56">
...
export default RestaurantGalleryPanel
